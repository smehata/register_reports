...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
	...
	invoice_pe_map = get_invoice_pe_map(invoice_list)
	invoice_pe_amt_map = get_invoice_pe_amt(invoice_list)
	...
	for inv in invoice_list:
		...
		payment_entry = list(set(invoice_pe_map.get(inv.name, {}).get("payment_entry", [])))
		paid_amount = list(set(invoice_pe_amt_map.get(inv.name, {}).get("paid_amount", [])))
		...
		if sales_order:
			...
			if len(sales_order) > 1:
				query += " and name in {0}".format(tuple(inv for inv in sales_order))
			else:
				query += " and name = '{0}'".format(sales_order[0])

			sales_order_amt = float(frappe.db.sql(query)[0][0] or 0)
		...
		row.update({
			...
			'sales_order_amt': sales_order_amt,
			'delivery_note': ", ".join(delivery_note),
			'payment_entry': ", ".join(payment_entry),
			'paid_amount': float(paid_amount[0]) if paid_amount else '',
...
def get_invoice_pe_map(invoice_list):
	payment_entry = frappe.db.sql("""select distinct pr.parent as py_entry, pr.reference_name as sales_invoice
			from `tabPayment Entry Reference` pr
			inner join `tabPayment Entry` as p on p.name = pr.parent
			where pr.reference_name in (%s) 
			and pr.reference_doctype ='Sales Invoice' and p.docstatus =1""" %
			', '.join(['%s'] * len(invoice_list)), tuple(inv.name for inv in invoice_list), as_dict=1)

	invoice_pe_map = {}
	for d in payment_entry:
		if d.sales_invoice:
			invoice_pe_map.setdefault(d.sales_invoice, frappe._dict()).setdefault(
				"payment_entry", []).append(d.py_entry)

	return invoice_pe_map

def get_invoice_pe_amt(invoice_list):

	payment_entry = frappe.db.sql("""select distinct sum(pr.allocated_amount) as paid_amount, 
		pr.reference_name as sales_invoice
		from `tabPayment Entry Reference` pr
		inner join `tabPayment Entry` as p on p.name = pr.parent 
		where pr.reference_name in (%s) 
	...
	invoice_pe_amount_map = {}
	for d in payment_entry:
		if d.sales_invoice:
			invoice_pe_amount_map.setdefault(d.sales_invoice, frappe._dict()).setdefault(
				"paid_amount", []).append(str(d.paid_amount))

	return invoice_pe_amount_map
